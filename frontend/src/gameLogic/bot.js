...
export function bot_play_piece(playerNames, player, difficulty) {
  switch (difficulty) {
    case "easy":
      easy_bot_play_piece(playerNames, player);
      break;
    case "medium":
      medium_bot_play_piece(playerNames, player);
      break;
    case "hard":
      hard_bot_play_piece(playerNames, player);
      break;
  }
}
...
function hard_bot_play_piece(playerNames, player) {
  let turn =
    player_pieces[player].filter((value) => value === false).length + 1;
  if (turn === 1) {
    // F piece
    let piece_index = 19;
    let rotations = 0;
