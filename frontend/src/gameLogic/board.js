...
export function play_random_piece(
  playerNames,
  player,
  greedySize = false,
  greedyCorners = false
    ...
    if (player_pieces[player][piece_index]) {
      ...
      for (let rotation = 0; rotation < 3; rotation++) {
        ...
        for (let boardRow = 0; boardRow < board_matrix.length; boardRow++) {
          for (
            let boardCol = 0;
            boardCol < board_matrix[boardRow].length;
            boardCol++
          ) {
            ...
            if (
              ...
              let playable_corners = find_playable_corners(
                piece_index,
                boardRow,
                boardCol,
                player
              );
              possible_piece_plays.push([
                boardRow,
                boardCol,
                rotation,
                playable_corners,
      ...
      if (possible_piece_plays.length > 0) {
        ...
        if (greedyCorners) {
          ...
          let possiblePlaysIndex = Math.floor(
            Math.random() * possible_piece_plays.length
          );
        ...
        for (
          let revertRotation = 0;
          revertRotation < playRotations;
          revertRotation++
        ) {
          rotate_piece(piece_index);
        }
...
function find_playable_corners(piece_index, boardRow, boardCol, player) {
  ...
  for (let r = 0; r < piece.length; r++)
    for (let c = 0; c < piece[r].length; c++)
      if (piece[r][c] === 1) board_matrix[boardRow + r][boardCol + c] = player;
  // loop through each piece
  for (let test_piece_index = 0; test_piece_index < 21; test_piece_index++) {
    // check if user has piece
    if (
      player_pieces[player][test_piece_index] &&
      test_piece_index !== piece_index
    ) {
      // loop through each rotation
      for (let rotation = 0; rotation < 3; rotation++) {
  ...
  for (let r = 0; r < piece.length; r++)
    for (let c = 0; c < piece[r].length; c++)
      if (piece[r][c] === 1) board_matrix[boardRow + r][boardCol + c] = "";
...
function end_round_checks(playerNames) {
  ...
  if (is_game_over()) {
    let end_players = determine_winner(
      playerNames,
      player_pieces,
      total_blocks_for_player,
      pieces_blocks_counts
    );
    if (in_online_game) game_over(lobby_code, end_players);
    else return end_players;
  } else {
    end_turn();
    reset_pieces();
  }
